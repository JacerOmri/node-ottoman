...
function _buildFilterExprs(filters, expressions, root) {
  var SPECIAL_KEYS = ['$exists', '$missing', '$contains', '$like', '$ilike', '$in'];
  var BOOLEAN = ['or', 'and'];
  var COMPARAISON = ['>', '>=', '<', '<='];
  if (!root) {
    root = '';
  ...
  for (var i in filters) {
    if (filters.hasOwnProperty(i)) {
      ...
      if (COMPARAISON.indexOf(i) > -1) {
        ident = root;
      } else {
        ident = root + '`' + i.split('.').join('`.`') + '`';
      }
      ...
      if (filters[i].$ilike) {
        expressions.push('lower(' + ident + ') LIKE ' + '\'' + filters[i].$ilike.toLowerCase() + '\'');
      }
      ...
      if (filters[i].$in) {
        ...
        expressions.push('ANY x IN ' + ident + ' SATISFIES ' +
        Object.values(subfilters).map(el => `x=\'${el}\'`).join(' AND ') + ' END');
      }
      if (BOOLEAN.indexOf(i.toLowerCase()) !== -1) {
        ...
        if (COMPARAISON.indexOf(Object.keys(filters[i])[0]) > -1) {
        ...
        if (typeof filters[i] === 'number' || typeof filters[i] === 'boolean') {
          switch (i) {
            ...
            case '>=':
            case '>':
              expressions.push(`${ident} ${i} ${filters[i]}`);
              break;
...
CbStoreAdapter.prototype.count = function (type, modelName, options, callback) {
  ...
  if (modelName == 'InfoPost') {
    whereQs += ' and -MILLIS(created_at) is valued';
  }

  var fullQs = 'SELECT COUNT(*) AS count FROM `' + bucketName + '` b' + whereQs;
  if (this.debug) {
...
CbStoreAdapter.prototype.find = function (type, modelName, options, callback) {
  ...
  if (modelName == 'InfoPost') {
    sortQs = ' and -MILLIS(created_at) is valued ORDER BY  -MILLIS(created_at)';
  }
